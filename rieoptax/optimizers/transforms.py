...
def scale_by_radam(b1 : float=0.9, b2: float=0.99, eps: float= 1e-8, eps_root: float =0.0) -> RiemannianGradientTransformation:
  ...
  def update_fn(updates, state, params, manifold_dict):
    ...
    nu = update_moment_per_elem_norm(updates, state.nu, b2, 2, manifold_dict, params)
